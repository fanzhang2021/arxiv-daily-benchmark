...
稳健版：时间窗 + ID去重 + 关键词过滤 + LLM Benchmark判定 + 分流式seen记录

流程：
抓取 arXiv 论文
→ ID去重
→ 时间窗过滤
→ 关键词过滤（cs.SE/cs.DC 豁免）
→ LLM Benchmark 判定
   - 不属于 benchmark：立即记录 seen
   - 属于 benchmark：LLM总结 → 邮件推送
       → 邮件成功：记录 seen
       → 邮件失败：不记录 seen（下次可重试）
"""
...
def paper_in_exempt_categories(paper: dict, exempt_categories: list[str]) -> bool:
    """检查论文分类是否在豁免列表中（cs.SE/cs.DC 可跳过关键词检查）"""
    return any(cat in exempt_categories for cat in paper.get("categories", []))
...
def fetch_papers(config: dict) -> tuple[list[dict], date, date]:
    ...
    logger.info(f"🔓 豁免关键词检查的分类: {exempt_cats}")
    ...
    for result in client.results(search):
        ...
        if keyword_mode == "filter" and keywords:
            if paper_in_exempt_categories(paper, exempt_cats):
...
def main():
    logger.info("=" * 60)
    logger.info("   🚀 ArXiv Daily Paper Digest (Benchmark专用版) 启动")
    logger.info("=" * 60)
    ...
    # ── 阶段 3：LLM Benchmark 判定 + 总结 ──
    logger.info(f"📝 开始处理 {len(papers)} 篇候选论文...")
    logger.info(f"🏷️ 只推送 benchmark 论文 (非 benchmark 将被过滤并记录 seen)")

    benchmark_papers = []      # 最终推送的论文 (benchmark)
    summaries = []             # 对应的摘要

    rejected_non_benchmark = []  # 非 benchmark，立即记 seen
    ...
    for i, p in enumerate(papers, 1):
        ...
        if not is_bench:
            # 非 benchmark → 立即记录 seen，不推送
            skipped_by_benchmark += 1
            rejected_non_benchmark.append(p["base_id"])
        ...
        # 是 benchmark → 生成摘要
        logger.info("    🎯 确认为 benchmark 论文，开始生成摘要")
    ...
    # 立即记录所有非 benchmark 论文
    if rejected_non_benchmark:
        save_seen_papers(rejected_non_benchmark)
        logger.info(f"🗂️ 已记录 {len(rejected_non_benchmark)} 篇非 benchmark 论文，下次不会重复判断")

    logger.info(f"📊 候选论文共 {len(papers)} 篇，其中 benchmark 论文 {len(benchmark_papers)} 篇，非 benchmark 跳过 {skipped_by_benchmark} 篇")

    if not benchmark_papers:
        logger.warning("📭 没有发现 benchmark 论文，任务结束。")
        return

    # ── 阶段 4：组装 HTML ──
    html = build_html(benchmark_papers, summaries, config, cutoff_date, today_date)
